...
def debug_materials():
    ...
    cur.execute(f"SELECT COUNT(*) as total FROM {settings.MATERIALS_TABLE}")
    total = cur.fetchone()['total']
    
    cur.execute(f"SELECT COUNT(*) as with_emb FROM {settings.MATERIALS_TABLE} WHERE description_embedding IS NOT NULL")
    with_emb = cur.fetchone()['with_emb']
    
    cur.execute(f"SELECT material_group, COUNT(*) as count FROM {settings.MATERIALS_TABLE} GROUP BY material_group ORDER BY count DESC")
